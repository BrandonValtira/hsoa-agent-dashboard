...
export function OfficeList() {
  ...
  }, [allOffices, brokerageFilter, searchQuery, getBrokerage]);
      ...
      <div className="space-y-4">
        ...
        <div className="flex flex-wrap gap-3">
          <select
            ...
            className="rounded-lg border border-[#e8e8e8] px-4 py-3 max-w-[220px] text-base text-[#3e4543] focus:border-[#832238] focus:outline-none focus:ring-1 focus:ring-[#832238] bg-white truncate h-[50px]"
          ...
          >
            ...
            {brokerageList.map((b) => (
              <option key={b.id} value={b.id} title={b.name}>
                {b.name.length > 28 ? b.name.slice(0, 25) + "…" : b.name}
              </option>
            ))}
          </select>
        </div>

        {(searchQuery.trim() || brokerageFilter) && (
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {filteredOffices.length === 0 ? (
          <p className="col-span-full py-8 text-center text-[#6b7270]">
            {searchQuery.trim() || brokerageFilter ? "No offices match your filters." : "No offices found."}
          </p>
        ) : (
